...
%define taglevel 642.6.2.vs230.web10027.xidmask.3
...
cp_vmlinux()
{
  %{strip_cmd} -R .note -R .comment -o "$2" "$1"
}
...
%if %{with_kabichk}
    ...
    # whitelist checking
    # if this one fails, the build fails
    ...
    # greylist checking
    # if this one fails, we go on with warnings in build log
    greylist=$RPM_SOURCE_DIR/Module.kabi_greylist_%{_target_cpu}$Flavour
    if [ -e $greylist ]; then
        echo "**** kABI: greylist checking start ****"
        $RPM_SOURCE_DIR/check-kabi -k $greylist -s Module.symvers
        echo "**** kABI: greylist checking stop  ****"
    fi
...
%endif
    ...
    cd include
    cp -a acpi config crypto keys linux math-emu media mtd net pcmcia rdma rxrpc scsi sound trace video drm uapi asm-generic $RPM_BUILD_ROOT/lib/modules/$KernelVer/build/include
    asmdir=$(readlink asm)
...
%endif
    ...
    for i in alias alias.bin ccwmap dep dep.bin ieee1394map inputmap isapnpmap ofmap pcimap seriomap symbols symbols.bin usbmap softdep
    do
...
%global perf_make \
  make %{?_smp_mflags} -C tools/perf -s V=1 NO_GTK2=1 prefix=%{_prefix} lib=%{_lib}
%if %{with_perf}
%{perf_make} all
...
# current kabi release directory
KABI_CURRENT=kabi-rhel68

mkdir -p $INSTALL_KABI_PATH

# install kabi releases directories
tar xjvf %{SOURCE38} -C $INSTALL_KABI_PATH

# add 'kabi-current' link
(cd $INSTALL_KABI_PATH; ln -s $KABI_CURRENT kabi-current)
...
then\
    (cd /usr/src/kernels/%{KVERREL}%{?1:.%{1}} &&\
     /usr/bin/find . -type f | while read f; do\
       hardlink -c /usr/src/kernels/*.el6*.*/$f $f\
     done)\
fi\
%{nil}
...
%kernel_variant_files %{with_debug} debug 1
...
%changelog
### PlanetLab
# Changelog removed temporarily -- makes the file too large for diff tools.
# Will be reinserted once working.
### PlanetLab
